...
These commands take a query string as an argument, make the query, let the user choose from the search results, then open the result in a web browser.
...
They also use Emacs' built-in annotation facilities (usually used for disambiguation or other information).
