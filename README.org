...
=musicbrainz-interactive= defines functions that make creating searching commands easier.

It provides search commands:
- =musicbrainz-interactive-search-annotation=
- =musicbrainz-interactive-search-area=
- =musicbrainz-interactive-search-artist=
- =musicbrainz-interactive-search-cdstub=
- =musicbrainz-interactive-search-event=
- =musicbrainz-interactive-search-instrument=
- =musicbrainz-interactive-search-label=
- =musicbrainz-interactive-search-place=
- =musicbrainz-interactive-search-recording=
- =musicbrainz-interactive-search-release=
- =musicbrainz-interactive-search-release-group=
- =musicbrainz-interactive-search-series=
- =musicbrainz-interactive-search-tag=
- =musicbrainz-interactive-search-url=
- =musicbrainz-interactive-search-work=

These commands take a query string as an argument, make the query and let the user choose from the search results and open them in a web browser.
They also have separate completion categories for every entity and use propertized strings (every candidate has their query data attached to them via a text property) which allows for integration with packages like [[https://github.com/oantolin/embark][embark]].
They also use Emacs' built-in annotation facilites (usually disambiguation and other information is put there).

These are built with helper functions that make working with the entities easier and can be used to make custom functions.

For example, this custom function will open the relationships page of a searched artist.
#+BEGIN_SRC emacs-lisp
(defun musicbrainz-interactive-search-artist-relationships
    (query &optional limit offset)
  "Enter a QUERY to search for an artist and open their relationships page.
...
descriptions."
  ...
  (musicbrainz-interactive-search
   ...
   (lambda (mbid)
     (browse-url
      (format "https://musicbrainz.org/artist/%s/relationships" mbid)))
   limit
   offset))
#+END_SRC
